<?php
			...
			session_start();
			$fname=mysqli_real_escape_string($con,$_POST['supplier_fname']);
			$lname=mysqli_real_escape_string($con,$_POST['supplier_lname']);
			 ...
			 empty($address))
			{
				...
				echo "<script>window.location.href='reg_Supplier_DB.php'</script>";
			...
			if(!preg_match($name,$fname))
				...
				$sql = "SELECT supplier_id FROM tbl_supplier WHERE email = '$email' LIMIT 1" ;
				...
				$rows = $row[0];
					...
					if($rows == 0)
					{	
						$insertSQL ="INSERT INTO tbl_supplier VALUES ('','$fname','$lname','$email','$mobile','$address','$password','$date')";
